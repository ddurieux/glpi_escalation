...
class PluginEscalationGroup_Group extends CommonDBRelation
{
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
   {
      ...
      if (
         ...
         if ($peConfig->getValue('workflow', $item->fields['entities_id']) == '1') {
            ...
            if (
               PluginEscalationProfile::haveRight("bypassworkflow", 1)
   ...
   function showGroups($tickets_id)
   {
      ...
      if (
         ...
         if (PluginEscalationProfile::haveRight("bypassworkflow", 1)) {
            ...
            $rand = Dropdown::showFromArray('group_assign', $a_groups, ['on_change' => 'Ext.get("useradd").hide();Ext.get("or").hide();']);
         }
         echo "</td>";
         ...
         $rand = Dropdown::showFromArray("_users_id_assign", $elements, ['on_change' => 'Ext.get("groupadd").hide();Ext.get("or").hide();']);
         echo "</td>";
         if ($createticketRight) {
            echo "<td>";
            ...
            echo "</td>";
            echo "<td>";
            Dropdown::show('Sla', ['entity' => $ticket->fields["entities_id"]]);
            echo "</td>";
         } else {
            echo "<td colspan='2'>";
            echo "</td>";
         ...
         Ajax::updateItemOnSelectEvent(
            "dropdown_group_assign" . $rand,
            "show_assignuser$rand",
   ...
   function manageGroup($groups_id)
   {
      ...
      Html::closeArrowMassives(['deleteitem' => __('Delete permanently')]);
      Html::closeForm();
   }
      ...
      if ($_SESSION['glpiactiveprofile']['interface'] == 'central') {
         ...
         if ($peConfig->getValue('workflow', $item->fields['entities_id']) == '1') {
            if (
               ...
               if (
                  isset($_POST['dropdown__groups_id_requester'])
